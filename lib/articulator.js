...
function Articulator(opts) {
  ...
  this.converse = (server, sessionId, message, context, reset) => {
  ...
  const continueRunActions = (server, sessionId, currentRequest, message, prevContext, i) => {
    return (json) => {
      if (i < 0) {
      ...
      if (json.type === 'msg') {
        ...
        return actions.send(server, request, response).then((ctx) => {
          ...
          return this.converse(server, sessionId, null, prevContext).then(
            continueRunActions(server, sessionId, currentRequest, message, prevContext, i - 1)
          );
        });
        ...
        return actions[action](request).then((ctx) => {
          ...
          return this.converse(server, sessionId, null, nextContext).then(
            continueRunActions(server, sessionId, currentRequest, message, nextContext, i - 1)
          );
        });
      } else {
  ...
  this.runActions = (server, sessionId, message, context, maxSteps) => {
    ...
    return this.converse(server, sessionId, message, context, currentRequest > 1).then(
      continueRunActions(server, sessionId, currentRequest, message, context, steps)
    ).then(cleanup);
  };
...
const throwMustHaveActions = () => {
  throw new Error('You must provide the `actions` parameter to be able to use runActions. ')
};
...
const validateActions = (logger, actions) => {
  ...
  if (!actions.send) {
    throw new Error('The \'send\' action is missing. ');
  }
  ...
  Object.keys(actions).forEach(key => {
    ...
    if (key === 'say' && actions[key].length > 2 ||
      ...
      logger.warn('The \'' + key + '\' action has been deprecated. ');
    }

    if (key === 'send') {
      if (actions[key].length !== 2) {
        logger.warn('The \'send\' action should accept 2 arguments: request and response. ');
      }
    } else if (actions[key].length !== 1) {
      logger.warn('The \'' + key + '\' action should accept 1 argument: request. ');
    }
  });
